            ...
            <div className="luxury-card p-4 rounded-xl border border-white/5 hover:border-primary/50 transition-all hover:-translate-y-1 group relative overflow-hidden">
                ...
                <div className="space-y-2 mb-4">
                    {item.signals.map((sig: string) => (
                        <div key={sig} className={`flex items-center justify-center text-xs font-bold px-3 py-2 rounded-lg border ${sig === "GOLDEN_CROSS" ? "bg-yellow-500/10 border-yellow-500/30 text-yellow-500" :
...
export default function HunterClient({ initialResults }: HunterClientProps) {
    ...
    return (
        <div className="animate-in fade-in duration-500 pb-20">
            ...
            {/* hero section */}
            <div className="luxury-card flex flex-col md:flex-row justify-between items-center gap-8 mb-12 p-10 relative overflow-hidden border-primary/20">
                <div className="relative z-10 space-y-2">
                    <h1 className="text-5xl text-white font-instrument tracking-tight">Signal Hunter</h1>

                    <p className="text-gray-400 max-w-xl text-sm leading-relaxed">
                        Scanning the ASX 200 for high-probability technical setups. Our engine identifies
                        <span className="text-white/60"> Whale movements</span>,
                        <span className="text-white/60"> Overbought/Oversold levels</span>,
                        and <span className="text-white/60">trend reversals</span> in real-time.
                    </p>
                </div>
                ...
                <div className="relative z-10 shrink-0">
                    ...
                    <div className="relative p-6 bg-surface/80 rounded-full border border-white/10 shadow-2xl backdrop-blur-xl">
                        {loading ? (
                            <Loader2 size={80} className="text-primary animate-spin" />
                        ) : (
                            <Target size={80} className="text-primary animate-pulse" />
                        )}
                    </div>
                </div>
