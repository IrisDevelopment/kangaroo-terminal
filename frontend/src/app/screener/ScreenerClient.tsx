...
interface ScreenerClientProps {
  initialMovers: any;
  initialMarketMap: any[];
}

export default function ScreenerClient({ initialMovers, initialMarketMap }: ScreenerClientProps) {
  const [movers, setMovers] = useState<any>(initialMovers);
  const [marketMap, setMarketMap] = useState<any[]>(initialMarketMap);
                        ...
                        <div className="flex items-center justify-between p-3 rounded-lg hover:bg-white/5 transition-colors group cursor-pointer border border-transparent hover:border-white/5">
                            ...
                            <div className="text-right">
                                ...
                                <span className="block text-xs font-bold text-success bg-success/10 px-2 py-0.5 rounded-full border border-success/20">
                                    {String(stock.change_percent).startsWith("-") || String(stock.change_percent).startsWith("+") 
